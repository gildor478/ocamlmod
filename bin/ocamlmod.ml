...
let dump_ml chn_out fn =
  ...
  let modname =
    String.capitalize_ascii (Filename.basename (Filename.chop_suffix fn ".ml"))
  in

  let export_extract chn_out fn =
    let is_rgxp s_rgxp =
      let rgxp = Str.regexp s_rgxp in
      fun s -> Str.string_match rgxp s 0
    in
    let with_odn = Str.regexp " +with +odn\\($\\| \\)" in
    let type_conv_path = Str.regexp "TYPE_CONV_PATH +\"[^\"]*\"" in
    let is_export_end = is_rgxp "(\\* +END +EXPORT +\\*)" in
    let is_export_start = is_rgxp "(\\* +START +EXPORT +\\*)" in
    let is_beg_comment = is_rgxp "(\\*.*" in
    ...
    let rec skip_header line =
      if is_export_start line then start_export (input_line ())
      else if is_beg_comment line then skip_header (input_line ())
      else start_export line
    and skip_export line =
      if is_export_start line then start_export (input_line ())
      else skip_export (input_line ())
    and start_export line =
      Printf.fprintf chn_out "# %d %S\n" !line_num fn;
      parse_body line
    and parse_body line =
      if is_export_end line then skip_export (input_line ())
      else
        let line =
          (* Remove ODN elements *)
          List.fold_left
        ...
        in
        if line = "" then Printf.fprintf chn_out "\n"
        else Printf.fprintf chn_out "  %s\n" line;
        parse_body (input_line ())
    in
    (try skip_header (input_line ()) with End_of_file -> ());
    close_in chn_in
  in
...
let process chn_in curdir chn_out =
  try
    while true do
      let fn = input_line chn_in in
      let real_fn =
        if Filename.is_relative fn then Filename.concat curdir fn else fn
      in
...
let () =
  ...
  Arg.parse
    [
      ("-o", Arg.String (fun str -> output := Some str), "fn Output file.");
      ( "-version",
        Arg.Unit
          (fun () ->
  ...
  | _, [] -> failwith "Need at least one .mod file."
  | Some out_fn, [ in_fn ] -> process_modfile ~out_fn in_fn
  | Some _, _ -> failwith "Cannot use -o with multiple .mod files."
  | None, lst -> List.iter process_modfile (List.rev lst)
